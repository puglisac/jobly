...
class User {
    ...
    static async get(username) {
        const user = await db.query(
            `
    SELECT username, first_name, last_name, email, photo_url, is_admin FROM users WHERE username = $1`, [username]
        );
        if (!user.rows[0]) {
            throw new ExpressError(`No such user: ${username}`, 404);
        }
        const u = user.rows[0]
        return new User(u.username, u.first_name, u.last_name, u.email, u.photo_url, u.is_admin);
    }
    async save() {
        await db.query(
