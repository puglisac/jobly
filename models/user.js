...
class User {
	...
	async save() {
		await db.query(
			`UPDATE users SET first_name=$2, last_name=$3, email=$4, photo_url=$5, is_admin=$6 WHERE username = $1`,
			[ this.username, this.first_name, this.last_name, this.email, this.photo_url, this.is_admin ]
		);
	}
	async remove() {
